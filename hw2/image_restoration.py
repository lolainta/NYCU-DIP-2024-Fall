...
def main():
    for i in range(2):
        img_original = cv2.imread(
            "data/image_restoration/testcase{}/input_original.png".format(i + 1)
        )
        img_blurred = cv2.imread(
            "data/image_restoration/testcase{}/input_blurred.png".format(i + 1)
        )
        ...
        print(
            "PSNR = {}\n".format(
                compute_PSNR(img_original, constrained_least_square_img)
            )
        )
